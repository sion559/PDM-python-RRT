...
class Quadrotor():
    def __init__(self, x=0, y=0, z=0, roll=0, pitch=0, yaw=0, size=0.25, show_animation=True, obstacleList=[]):
    ...
    def plot(self):  # pragma: no cover
        ...
        for (ox, oy, oz, size) in self.obstacleList:
            ...
            x = size*np.cos(u)*np.sin(v)
            y = size*np.sin(u)*np.sin(v)
            z = size*np.cos(v)
            self.ax.plot_wireframe(x+ox, y+oy, z+oz, color="b")

        if not self.plotOnce:
            self.plotOnce = True
            plt.xlim(-10, 10)
            plt.ylim(-10, 10)
